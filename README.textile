h1. Beanstalk + Tokyo Tyrant = Edamame

"Edamame":http://bit.ly/edamame combines the "Beanstalk priority queue":http://bit.ly/beanstalkd with a "Tokyo Tyrant database"::http://bit.ly/ttyrant and "God monitoring":http://bit.ly/godmonitor to produce a persistent lightweight networked queuing system.

It includes  scripts for monitoring and failover, and a few other nifty toys.
...
* a set of God monitoring (http://god.rubyforge.org/) scripts for managing the daemons
* command-line utilities to load. enumerate, empty, and show stats for the db+queue
* the start of a lightweight sinatra to monitor the queue (this needs work...)
...
* Restarting is still manual
* Right now jobs live in full in the beanstalkd. This carries a heavy memory cost but a performance gain (the alternative is to do a second query to the DB once the job has been retrieved). I'm going to make both implementations available.
